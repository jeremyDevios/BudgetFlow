    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-2xl animate-in slide-in-from-bottom-10 duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Montant */}
          <div>
            <label className="block text-sm font-medium text-zinc-400 mb-1">Montant</label>
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-black border border-zinc-800 rounded-xl py-4 pl-10 pr-4 text-2xl font-bold text-white focus:ring-2 focus:ring-amber-500 focus:outline-none"
                    placeholder="0.00"
                    autoFocus
                />
            </div>
          ...
          {/* Enveloppe */}
          <div>
             <label className="block text-sm font-medium text-zinc-400 mb-1">Enveloppe</label>
             <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-zinc-700">
                {envelopes.map(env => (
                    <button
                        ...
                        className={`p-3 rounded-lg border text-left flex items-center gap-2 transition-all ${selectedEnvelopeId === env.id ? 'bg-zinc-800 border-amber-500 ring-1 ring-amber-500' : 'bg-black border-zinc-800 hover:bg-zinc-900'}`}
                    >
                        <div className={`w-3 h-3 rounded-full ${env.color}`} />
                        <span className={`truncate text-sm ${selectedEnvelopeId === env.id ? 'text-white font-medium' : 'text-zinc-400'}`}>{env.name}</span>
                    </button>
                ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
                ...
                <input
                    ...
                    className="w-full bg-black border border-zinc-800 rounded-lg p-3 text-white focus:ring-1 focus:ring-amber-500 focus:outline-none"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-4 bg-amber-500 hover:bg-amber-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-colors"
          >
